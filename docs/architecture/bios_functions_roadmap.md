...
- [x] `0x19` — `HookEntryInt(addr)` — _Functional: installs the `HookEntryInt` descriptor, preserves longjmp-style resumed callback context inside the callback bridge, and restores interrupted CPU registers after IRQ delivery_
