...
- COP0 now mirrors IRQ-controller pending state into `Cause.IP2`, keeps `mtc0 Cause` limited to software IP bits, and gates IRQ delivery/exception entry on `Status.IEc` + `Status.IM2` (while preserving runtime callback/critical-section guards).
- Boot now applies minimal COP0 defaults before handing control to the game (`Status.IEc=1`, `Status.IM2=1`, `Status.KUc=0`) so BIOS-style IRQ paths stay active in demos.
