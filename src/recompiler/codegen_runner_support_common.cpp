...
void emitRunnerSupportCommon(CppEmitter& emitter)
{
    ...
    emitter.writeBlank();
    emitter.writeLine("std::vector<psxrecomp::u16> captureBestDisplayPixels(const "
                      "psxrecomp::runtime::Gpu& gpu,");
    emitter.writeLine("                                              size_t* outWidth,");
    emitter.writeLine("                                              size_t* outHeight)");
    emitter.openBlock("");
    emitter.writeLine("if (outWidth == nullptr || outHeight == nullptr)");
    ...
    emitter.writeLine("return {};");
    emitter.closeBlock();
    emitter.writeLine("const auto framePixels = gpu.frameBufferSnapshot();");
    emitter.writeLine("const auto displayWindow = gpu.displayWindow();");
    emitter.writeLine("std::vector<psxrecomp::u16> displayPixels =");
    emitter.writeLine("    extractDisplayPixels(framePixels, displayWindow, outWidth, outHeight);");
    emitter.writeLine("size_t nonZeroDisplay = countNonZeroPixels(displayPixels);");
    emitter.writeLine("if (nonZeroDisplay == 0 && *outWidth > 0 && *outHeight > 0)");
    emitter.openBlock("");
    ...
    emitter.writeLine("if (static_cast<size_t>(displayWindow.y) + *outHeight < fullHeight)");
    emitter.openBlock("");
    emitter.writeLine("altWindow.y = static_cast<psxrecomp::u16>(displayWindow.y + *outHeight);");
    emitter.writeLine("hasAlternate = true;");
    emitter.closeBlock();
    emitter.writeLine("else if (static_cast<size_t>(displayWindow.y) >= *outHeight)");
    emitter.openBlock("");
    emitter.writeLine("altWindow.y = static_cast<psxrecomp::u16>(displayWindow.y - *outHeight);");
    emitter.writeLine("hasAlternate = true;");
    emitter.closeBlock();
    ...
    emitter.writeLine("if (altWidth == *outWidth && altHeight == *outHeight)");
    emitter.openBlock("");
    ...
    emitter.writeLine("if (nonZeroDisplay == 0 && *outWidth > 0 && *outHeight > 0)");
    emitter.openBlock("");
    emitter.writeLine("const auto& vramWords = gpu.vramWords();");
    emitter.writeLine("if (!vramWords.empty())");
    emitter.openBlock("");
    emitter.writeLine("const size_t fullWidth = psxrecomp::runtime::SoftwareGpuRenderer::Width;");
    emitter.writeLine("displayPixels.assign(*outWidth * *outHeight, 0);");
    emitter.writeLine("for (size_t y = 0; y < *outHeight; ++y)");
    emitter.openBlock("");
    emitter.writeLine("for (size_t x = 0; x < *outWidth; ++x)");
    emitter.openBlock("");
    ...
    emitter.writeLine("displayPixels[y * *outWidth + x] = pixel;");
    emitter.closeBlock();
    emitter.closeBlock();
    emitter.closeBlock();
    emitter.closeBlock();
    emitter.writeLine("return displayPixels;");
    emitter.closeBlock();
    emitter.writeBlank();
    emitter.writeLine("size_t countNonZeroPixels(const std::vector<psxrecomp::u16>& pixels)");
    emitter.openBlock("");
