...
bool PsxSystem::deserializeState(const std::vector<u8>& state)
{
    ...
    if (cursor != state.size())
    {
        if (!consumeU32(state, cursor, gteStateSize) || !readBlob(gteStateSize, gteStateCopy))
        {
            return false;
        }
        hasGteState = true;
...
void PsxSystem::callBiosSyscall(u32 code, u32* regs, size_t regCount)
{
    ...
    case 0x00:
    {
        ...
        if (subcommand == 1)
        {
            // EnterCriticalSection: return prior interrupt-enable state (1=was enabled, 0=was
            // disabled).
            regs[2] = (m_criticalSectionDepth > 0) ? 0u : 1u;
        ...
        if (subcommand == 2)
        {
            // ExitCriticalSection: return 1 if depth decremented, 0 if already at 0.
