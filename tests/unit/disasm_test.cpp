...
int main()
{
    ...
    {
        [[maybe_unused]] Instruction add =
            MipsDisassembler::decode(encodeR(1, 2, 3, 0, 0x20), 0x80010000);
        ...
        [[maybe_unused]] Instruction move =
            MipsDisassembler::decode(encodeR(0, 9, 8, 0, 0x25), 0x80010002);
        assert(move.opcode == Opcode::OR);
        assert(move.toString() == "move $t0, $t1");
    }

    {
        [[maybe_unused]] Instruction sll =
            MipsDisassembler::decode(encodeR(0, 9, 8, 4, 0x00), 0x80010004);
        assert(sll.opcode == Opcode::SLL);
        assert(sll.shamt == 4);
        assert(sll.toString() == "sll $t0, $t1, 4");

        [[maybe_unused]] Instruction nop =
            MipsDisassembler::decode(encodeR(0, 0, 0, 0, 0x00), 0x80010006);
        assert(nop.opcode == Opcode::SLL);
        assert(nop.toString() == "nop");
    }

    {
        [[maybe_unused]] Instruction addi =
            MipsDisassembler::decode(encodeI(0x08, 4, 2, -16), 0x80010008);
        assert(addi.opcode == Opcode::ADDI);
        assert(addi.immediate == -16);
        assert(addi.toString() == "addi $v0, $a0, -16");

        [[maybe_unused]] Instruction move =
            MipsDisassembler::decode(encodeI(0x09, 9, 8, 0), 0x8001000A);
        assert(move.opcode == Opcode::ADDIU);
        assert(move.toString() == "move $t0, $t1");
    }

    {
        [[maybe_unused]] Instruction andi =
            MipsDisassembler::decode(encodeI(0x0C, 4, 2, 0x00FF), 0x8001000C);
        assert(andi.opcode == Opcode::ANDI);
        assert(andi.toString() == "andi $v0, $a0, 0x00FF");

        [[maybe_unused]] Instruction li =
            MipsDisassembler::decode(encodeI(0x0D, 0, 8, 0x00FF), 0x8001000E);
        assert(li.opcode == Opcode::ORI);
        assert(li.toString() == "li $t0, 0x00FF");
    }

    {
        [[maybe_unused]] Instruction lui =
            MipsDisassembler::decode(encodeI(0x0F, 0, 8, 0x1234), 0x80010010);
...

    {
        ...
        [[maybe_unused]] auto target = branch.getTargetAddress();
    ...
    {
        [[maybe_unused]] Instruction beqz =
            MipsDisassembler::decode(encodeI(0x04, 8, 0, 1), 0x80010050);
        assert(beqz.toString() == "beqz $t0, 0x80010058");

        [[maybe_unused]] Instruction bnez =
            MipsDisassembler::decode(encodeI(0x05, 8, 0, 1), 0x80010054);
        assert(bnez.toString() == "bnez $t0, 0x8001005C");

        [[maybe_unused]] Instruction b =
            MipsDisassembler::decode(encodeI(0x04, 8, 8, 1), 0x80010058);
        assert(b.toString() == "b 0x80010060");
    }

    {
        [[maybe_unused]] Instruction regimm =
            MipsDisassembler::decode(encodeI(0x01, 8, 0x10, 4), 0x80010018);
        assert(regimm.opcode == Opcode::BLTZAL);
        assert(regimm.isCall());
    }

    {
        [[maybe_unused]] Instruction bltz =
            MipsDisassembler::decode(encodeI(0x01, 9, 0x00, 8), 0x8001001A);
        assert(bltz.opcode == Opcode::BLTZ);
        assert(!bltz.isCall());
        assert(bltz.toString() == "bltz $t1, 0x8001003E");

        [[maybe_unused]] Instruction bgez =
            MipsDisassembler::decode(encodeI(0x01, 10, 0x01, -4), 0x8001001E);
        assert(bgez.opcode == Opcode::BGEZ);
        assert(!bgez.isCall());
        assert(bgez.toString() == "bgez $t2, 0x80010012");

        [[maybe_unused]] Instruction tgei =
            MipsDisassembler::decode(encodeI(0x01, 8, 0x08, -1), 0x80010040);
        assert(tgei.opcode == Opcode::TGEI);
        assert(tgei.toString() == "tgei $t0, -1");

        [[maybe_unused]] Instruction tltiu =
            MipsDisassembler::decode(encodeI(0x01, 9, 0x0B, 7), 0x80010044);
...

    {
        ...
        [[maybe_unused]] auto target = jump.getTargetAddress();
        assert(target.has_value());
        assert(*target == 0x8048D158);
        assert(jump.toString() == "j 0x8048D158");

        [[maybe_unused]] auto jumpTarget = jump.getJumpTarget();
        assert(jumpTarget.has_value());
        assert(*jumpTarget == 0x8048D158);
    }

    {
        [[maybe_unused]] Instruction jr =
            MipsDisassembler::decode(encodeR(31, 0, 0, 0, 0x08), 0x8001001C);
        assert(jr.opcode == Opcode::JR);
        assert(jr.isJump());
        assert(jr.isReturn());
        assert(jr.toString() == "jr $ra");

        [[maybe_unused]] Instruction jalr =
            MipsDisassembler::decode(encodeR(9, 0, 8, 0, 0x09), 0x80010020);
        assert(jalr.opcode == Opcode::JALR);
        assert(jalr.isCall());
        assert(jalr.toString() == "jalr $t0, $t1");

        [[maybe_unused]] Instruction jalrRa =
            MipsDisassembler::decode(encodeR(9, 0, 31, 0, 0x09), 0x80010020);
        assert(jalrRa.opcode == Opcode::JALR);
        assert(jalrRa.toString() == "jalr $t1");

        [[maybe_unused]] Instruction syscall =
            MipsDisassembler::decode(encodeR(0, 0, 0, 0, 0x0C), 0x80010024);
        assert(syscall.opcode == Opcode::SYSCALL);
        assert(syscall.toString() == "syscall");

        [[maybe_unused]] Instruction syscallWithCode =
        ...
        [[maybe_unused]] Instruction brk =
            MipsDisassembler::decode(encodeR(0, 0, 0, 0, 0x0D), 0x8001002C);
        assert(brk.opcode == Opcode::BREAK);
        assert(brk.toString() == "break");

        [[maybe_unused]] Instruction breakWithCode =
...

    {
        ...
        [[maybe_unused]] Instruction bc0f =
            MipsDisassembler::decode(encodeI(0x10, 0x08, 0x00, 2), 0x8001003C);
        assert(bc0f.opcode == Opcode::BC0F);
        assert(bc0f.toString() == "bc0f 0x80010048");

        [[maybe_unused]] Instruction bc0t =
            MipsDisassembler::decode(encodeI(0x10, 0x08, 0x01, -1), 0x80010040);
        assert(bc0t.opcode == Opcode::BC0T);
        assert(bc0t.toString() == "bc0t 0x80010040");

        [[maybe_unused]] Instruction lwc0 =
            MipsDisassembler::decode(encodeI(0x30, 8, 4, 12), 0x80010044);
        assert(lwc0.opcode == Opcode::LWC0);
        assert(lwc0.toString() == "lwc0 $c4, 12($t0)");

        [[maybe_unused]] Instruction swc0 =
            MipsDisassembler::decode(encodeI(0x38, 9, 5, -8), 0x80010048);
        assert(swc0.opcode == Opcode::SWC0);
        assert(swc0.toString() == "swc0 $c5, -8($t1)");
    }

    {
        [[maybe_unused]] Instruction mfc2 = MipsDisassembler::decode(
            (0x12u << 26) | (0u << 21) | (3u << 16) | (5u << 11), 0x80010026);
        assert(mfc2.opcode == Opcode::MFC2);
        assert(mfc2.toString() == "mfc2 $v1, $vz2");

        [[maybe_unused]] Instruction mtc2 = MipsDisassembler::decode(
            (0x12u << 26) | (4u << 21) | (9u << 16) | (7u << 11), 0x8001002A);
        assert(mtc2.opcode == Opcode::MTC2);
        assert(mtc2.toString() == "mtc2 $t1, $otz");

        [[maybe_unused]] Instruction cfc2 = MipsDisassembler::decode(
            (0x12u << 26) | (2u << 21) | (10u << 16) | (4u << 11), 0x8001002E);
        assert(cfc2.opcode == Opcode::CFC2);
        assert(cfc2.toString() == "cfc2 $t2, $r33");

        [[maybe_unused]] Instruction gteRtps = MipsDisassembler::decode(0x4A180001u, 0x80010032);
        assert(gteRtps.opcode == Opcode::GTE_RTPS);
        assert(gteRtps.toString() == "rtps");

        [[maybe_unused]] Instruction gteRtpt = MipsDisassembler::decode(0x4A280030u, 0x80010036);
        assert(gteRtpt.opcode == Opcode::GTE_RTPT);
        assert(gteRtpt.toString() == "rtpt");
    }

    {
        [[maybe_unused]] Instruction lwc2 =
            MipsDisassembler::decode(encodeI(0x32, 4, 6, 12), 0x80010028);
        assert(lwc2.opcode == Opcode::LWC2);
        assert(lwc2.toString() == "lwc2 $rgbc, 12($a0)");

        [[maybe_unused]] Instruction swc2 =
            MipsDisassembler::decode(encodeI(0x3A, 5, 7, -4), 0x8001002C);
        assert(swc2.opcode == Opcode::SWC2);
        assert(swc2.toString() == "swc2 $otz, -4($a1)");
    }

    {
        [[maybe_unused]] Instruction lwl =
            MipsDisassembler::decode(encodeI(0x22, 4, 8, 2), 0x80010040);
        assert(lwl.opcode == Opcode::LWL);
        assert(lwl.toString() == "lwl $t0, 2($a0)");

        [[maybe_unused]] Instruction lwr =
            MipsDisassembler::decode(encodeI(0x26, 4, 8, -2), 0x80010044);
        assert(lwr.opcode == Opcode::LWR);
        assert(lwr.toString() == "lwr $t0, -2($a0)");

        [[maybe_unused]] Instruction swl =
            MipsDisassembler::decode(encodeI(0x2A, 4, 8, 6), 0x80010048);
        assert(swl.opcode == Opcode::SWL);
        assert(swl.toString() == "swl $t0, 6($a0)");

        [[maybe_unused]] Instruction swr =
            MipsDisassembler::decode(encodeI(0x2E, 4, 8, -6), 0x8001004C);
        assert(swr.opcode == Opcode::SWR);
        assert(swr.toString() == "swr $t0, -6($a0)");

        [[maybe_unused]] Instruction cache =
            MipsDisassembler::decode(encodeI(0x2F, 8, 0x1F, 16), 0x80010050);
    ...
    {
        [[maybe_unused]] Instruction unknown = MipsDisassembler::decode(0xFC000000u, 0x80010038);
        assert(unknown.opcode == Opcode::UNKNOWN);
        assert(unknown.toString() == "unknown 0xFC000000");
    }

    {
        [[maybe_unused]] Instruction neg =
            MipsDisassembler::decode(encodeR(0, 9, 8, 0, 0x22), 0x80010060);
        assert(neg.toString() == "neg $t0, $t1");

        [[maybe_unused]] Instruction negu =
            MipsDisassembler::decode(encodeR(0, 9, 8, 0, 0x23), 0x80010064);
        assert(negu.toString() == "negu $t0, $t1");

        [[maybe_unused]] Instruction notInstr =
            MipsDisassembler::decode(encodeR(9, 0, 8, 0, 0x27), 0x80010068);
        assert(notInstr.toString() == "not $t0, $t1");
    }

    {
        Instruction backward = MipsDisassembler::decode(encodeI(0x05, 8, 9, -1), 0x80010050);
        [[maybe_unused]] auto target = backward.getTargetAddress();
        assert(target.has_value());
    ...
    {
        [[maybe_unused]] Instruction load =
            MipsDisassembler::decode(encodeI(0x23, 8, 9, 4), 0x80010070);
        ...
        [[maybe_unused]] Instruction branch =
            MipsDisassembler::decode(encodeI(0x05, 8, 9, 2), 0x80010074);
        assert(branch.getAddressingMode() == psxrecomp::disasm::AddressingMode::PC_RELATIVE);
        assert(branch.getBranchTarget() == 0x80010080);

        [[maybe_unused]] Instruction jump =
            MipsDisassembler::decode(encodeJ(0x02, 0x00100000), 0x80010078);
        assert(jump.getAddressingMode() == psxrecomp::disasm::AddressingMode::ABSOLUTE);

        [[maybe_unused]] Instruction jr =
            MipsDisassembler::decode(encodeR(31, 0, 0, 0, 0x08), 0x8001007C);
...

    {
        ...
        {
            [[maybe_unused]] Instruction invalid =
